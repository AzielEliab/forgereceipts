...
export const MESH_PRODUCT = "forgereceipts";
...
export function meshOpenApiPaths() {
  ...
  for (const route of MESH_PROXY_ROUTES) {
    ...
    for (const method of route.methods) {
      entry[method] = {
        operationId: "forgereceipts_mesh_" + route.op + (method === "head" ? "_head" : "") + "_proxy",
...
export function isMeshPath(pathname) {
  ...
  return path === MESH_PATH || path.startsWith(MESH_PATH + "/");
}

export function meshOrigin(env) {
...
export function meshTargetUrl(pathname, requestUrl, env) {
  if (!isMeshPath(pathname)) return null;
  const path = String(pathname || "").replace(/\/+$/, "") || "/";
  let search = "";
  try {
  ...
  return meshOrigin(env) + path + search;
}
...
/** MESH-OK envelope. Radios stay OFF. GET never enables. */
export function meshOkFallback(extra = {}) {
  const rollup = extra.rollup && typeof extra.rollup === "object"
    ? { ...emptyRollup(), ...extra.rollup }
    : emptyRollup();
  return attachQnsCd({
    ...
    enabled: extra.enabled === true ? true : false,
    author: MESH_IDENTITY,
    identity: MESH_IDENTITY,
...
function meshOpFromPath(pathname) {
  const path = String(pathname || "").replace(/\/+$/, "") || "/";
  if (path === MESH_PATH || path === MESH_STATUS_PATH) return "status";
  if (path.startsWith(MESH_PATH + "/")) return path.slice(MESH_PATH.length + 1) || "status";
  return "status";
}

/**
 * PROXY /v1/mesh/* to aziel-runtime via AZIEL_RUNTIME when present.
 * HTTP fallback to https://aziel-runtime.vibelock.workers.dev.
 * GET/HEAD status stays MESH-OK + enabled:false when upstream is unavailable.
 */
export async function handleMesh(request, url, env) {
  if (!isMeshPath(url.pathname)) return null;
  const dest = meshTargetUrl(url.pathname, request.url, env);
  ...
  try {
    const fetcher = env && env.AZIEL_RUNTIME && typeof env.AZIEL_RUNTIME.fetch === "function"
      ? env.AZIEL_RUNTIME
      : null;
    ...
    if (method === "GET" || method === "HEAD") {
      return meshJson(meshOkFallback({
        op,
        source: "fallback",
        detail: String(exc).slice(0, 240),
        origin: dest,
        note: MESH_NOTE + " Upstream unavailable; radios stay OFF.",
      }));
    }
    return meshJson({
      ok: false,
      code: "MESH-PROXY-FAILED",
      enabled: false,
      default_off: true,
      author: MESH_IDENTITY,
      identity: MESH_IDENTITY,
